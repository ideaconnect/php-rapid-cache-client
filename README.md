...
`RapidCacheClient` is benchmarked against [Symfony Cache](https://symfony.com/doc/current/components/cache.html)
(Redis adapter, with both igbinary and JSON serialization) for plain and tagged
SET/GET throughput. The chart is regenerated by CI and published to the
`assets` branch:

![Benchmark results](https://raw.githubusercontent.com/ideaconnect/php-rapid-cache-client/assets/benchmark.svg)

The chart reports three throughput metrics per adapter, all in operations per
second (higher is better):

- **SET ops/sec** - items written per second (`itemCount / setTime`).
- **GET ops/sec** - items read per second (`itemCount / getTime`).
- **Total ops/sec** - combined write-and-read throughput. Each item is both
  written once and read once, so this is `(itemCount × 2) / totalTime`, where
  `totalTime` is the elapsed time of the SET and GET phases together. It is the
  single headline figure for "how many cache operations per second can this
  adapter sustain across a full round trip", not a sum of the two bars above.

> **A fair word about this comparison.** This is **not** a claim that Symfony
> Cache is worse - it isn't. Symfony Cache is a far more robust, general-purpose
> caching component: it supports many backends, cache pools, PSR-6 + PSR-16,
> stampede protection, deferred saves, adapter chaining, and much more. That
> breadth carries overhead, and it fully earns the right to be a little slower.
>
> The benchmark only measures the **narrow slice of functionality this library
> covers** - direct, no-frills Redis caching. Within that slice, and only that
> slice, `RapidCacheClient`'s lean, igbinary-serialized, pipelined approach
> comes out faster. If you need the wider feature set Symfony Cache provides,
> use Symfony Cache; if you want the fastest possible path for plain Redis
> caching, that is exactly what this library optimizes for.

> Absolute numbers depend on the runner hardware - treat the chart as a relative
> comparison between adapters for this specific use case, not a guaranteed
> throughput figure.

Reproduce locally from the [`benchmark/`](benchmark/) directory:
